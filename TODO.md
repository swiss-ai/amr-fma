...
- Implement `core.models`:
  ...
  - preparing for LoRA+SFT and full SFT on 8B models.
...
- Choose one 8B instruct model family (e.g. Llama‑3.1‑8B or one Apertus 8B instruct model as in the proposal).[file:1]
- Choose one **medical advice** dataset from the grant:
  - e.g. `kabariap/II-Medical-Reasoning-SFT` or `lavita/ChatDoctor-HealthCareMagic-100k`.[file:2]
...
- Configure a Hydra experiment (e.g. `p1_temporal_detection_minimal`) that:
  ...
  - writes a complete `manifest.yaml`.
...
## Milestone 4 – Basic checkpoint evaluation (general + AMR-like)

**Goal:** Evaluate the pilot run at all checkpoints with one general and one AMR-relevant metric.

- Integrate vLLM or HF generation into `eval` for inference.
- Add loaders for at least:
  - one general benchmark (e.g. small subset of MMLU-Pro or IFEval), and
  - one simple AMR-like evaluation (e.g. refusal rate on a small harmfulness subset, loosely mirroring Safety Gap’s refusal + StrongREJECT direction).[file:1][cite:18]
...
**Completion signal:** You can plot (even manually) a curve of “general metric vs training fraction” and “AMR proxy vs training fraction” for the minimal LoRA+SFT run.
...
- Add support for a **code generation** domain using grant datasets:
  - e.g. `ise-uiuc/Magicoder-OSS-Instruct-75K` and/or `HuggingFaceH4/CodeAlpaca_20K`.[file:2]
- Implement or configure **full SFT** trainer for the same 8B model family.
...
**Completion signal:** You have four working trajectories (model × {medical, code} × {LoRA+SFT, full SFT}) with manifests and basic eval curves.
...
- Implement an offline SDPO trainer based on the SDPO procedure from the paper, integrated with your `RunManifest` and checkpoint logic (rather than hand-specified checkpoint lists).[cite:22]
...
**Completion signal:** You can compare LoRA+SFT vs full SFT vs SDPO trajectories (same model + domain) in terms of general metric vs AMR proxy over checkpoints.

---

## Milestone 7 – P1 “mini grid” across model families

**Goal:** Run a pruned but real subset of the full P1 grid from the proposal.[file:1]
...
- Select a smaller set of base tasks:
  ...
  - both code datasets (`Magicoder-OSS-Instruct-75K`, `CodeAlpaca_20K`).[file:2]
- For each selected combination, run:
  - LoRA+SFT and full SFT (SDPO at least for one family/domain if compute permits),
  - with a consistent checkpoint schedule and 2–3 seeds (as compute allows within the 50k GPUh budget).[file:1]
- Extend the eval pipeline to:
  - include a richer AMR suite (e.g. a couple of deception, sycophancy, evaluation awareness tests, as available),
  - still keep general capability metrics from Milestone 4.

**Completion signal:** You have a small but representative dataset of trajectories across model families, domains, and FMA methods, with comparable checkpoint-level capability and AMR metrics.
...
**Completion signal:** You have at least one mitigated run where you can compare “before vs after mitigation” AMR metrics at the same or later training fractions.
...
- Choose a handful of P1 runs where AMR drift is most evident (e.g. LoRA+SFT on medical or a specific SDPO run).[file:1]
- For each, pick ~3 intervention checkpoints (early, mid, late drift) as in the grant’s mitigation plan.[file:2]
- For each intervention:
  ...
  - evaluate pre/post-mitigation across the AMR and general metrics.
...
**Goal:** Demonstrate that the pipeline transfers to 32B in a limited but realistic scenario.[file:1]
...
- Run a small number of trajectories and evaluate them with the existing pipelines.
...
This roadmap keeps the early steps small and concrete, and splits the later work into more manageable milestones (SDPO integration, multi-family P1 mini grid, P2 scaffolding, P2 experiments, P3 pilot). Each milestone mentions the **models, methods, and datasets** from the grant, so it reads like a direct operationalisation of your proposal.